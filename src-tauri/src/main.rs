...
async fn get_status(state: State<'_, AppState>) -> Result<BridgeStatus, String> {
    ...
    let printer_online    = tokio::task::spawn_blocking(move || {
        check_printer_online(&printer_ip)
    })
    .await
    .unwrap_or(false);
...
fn test_print(state: State<AppState>) -> Result<(), String> {
    ...
    let ip = {
        let db = &state.db;
        db::config_get(db, "printer_ip").unwrap_or_default()
    };

    if ip.is_empty() {
        return Err("Printer IP not configured".into());
    }
...
fn main() {
    ...
    // Tokio runtime — kept alive for the entire process
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Pre-fill Supabase defaults if not already set
            if db::config_get(&db, "supabase_url").is_none() {
                db::config_set(
                    &db,
                    "supabase_url",
                    "https://rfquwezkkdyvjftveilf.supabase.co",
                )
                .ok();
            }
            ...
            rt.spawn(async move {
                ws_server::start(db_ws, pip_ws).await;
            });

            // Start Supabase sync loop
            let db_sync = db.clone();
            rt.spawn(async move {
                supabase::start_sync_loop(db_sync).await;
            });

            // Start mDNS
            rt.spawn(async move {
                mdns::start_mdns("VenoApp Bridge");
            });

            // Auto-show window on first run if setup is not complete
            let setup_done = db::config_get(&db, "setup_complete")
                .map(|v| v == "true")
                .unwrap_or(false);
